...
**O cliente traduz na borda, e só lá.** "O cliente lê os campos do original" vale para o *fio*: o front não ganha um evento de domínio vindo da API. Mas o desembrulho do envelope LC tem que acontecer em algum lugar, e o lugar escolhido é a borda do transporte (`apps/web/src/transporte/sse.ts`), que também renomeia os campos para o vocabulário do `CONTEXT.md` e parseia o JSON que o `ToolMessage` carrega como string. O reducer, a partir dali, nunca vê `{lc:1,...}`. Isso não reabre a decisão — nada disso atravessa a API nem o port —, mas é bom dizer em voz alta que existe uma tradução, e que ela é de um lado só: o front nunca serializa de volta.
